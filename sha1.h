...
void SHA1( const char *key, char *filename)
{
	//function for HMAC
	...
	gcry_md_hd_t hmac;
	size_t hashlen = gcry_md_get_algo_dlen(GCRY_MD_SHA1);	 // Retrieve the length in bytes of the digest yielded by SHA1
	...
	*/
	...
	gpg_error_t error;
	   ...
	   gcry_md_open (&hmac, GCRY_MD_SHA1, GCRY_MD_FLAG_HMAC); 	// Create a message digest object for SHA1
	   gcry_md_setkey (hmac, key, key_l);						// Set the key for the MAC
	   gcry_md_write (hmac, input, fileLength);					
	   hash = gcry_md_read (hmac, GCRY_MD_SHA1);
